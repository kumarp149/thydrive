...
$ignore=array('.','..','.htaccess','index.php','icon.php','Thumbs.db','web.config','files1.php','process.php'); // ignore these files
// End configs
...
while(false!==($f=readdir($h)))
{
    ...
    if(is_dir($path.$f))$dirs[]=array('name'=>$f,'date'=>filemtime($path.$f),'url'=>'files1.php?dir='.rawurlencode(trim("$dir/$f",'/')));
...
$up_dir=dirname($dir);
$up_url=($up_dir!=''&&$up_dir!='.')?'files1.php?dir='.rawurlencode($up_dir):'files1.php';
...
<head>
    ...
    function _f(name,size,date,url,rdate){_files[_files.length]={'dir':0,'name':name,'size':size,'date':date,'type':_ge(name),'url':url,'rdate':rdate,'icon':'files1.php?icon='+_ge(name)};_tsize+=size;}
    function _d(name,date,url){_dirs[_dirs.length]={'dir':1,'name':name,'date':date,'url':url,'icon':'files1.php?icon=dir'};}
